...
for dim in df['dimension'].unique():
    ...
    if len(valid_groups) < 2:
        print(f"Skipping dimension {dim}: not enough replicates per noise level for PERMANOVA")
        continue
    ...
    if res['p-value'] < 0.05:
        print("Reject H0: topology is not robust to noise")
    else:
        print("Fail to reject H0: topology is robust to noise")
